...
export default function IntegrationsPage() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      before?.is_enabled !== after?.is_enabled
    ) {
      setInspecting(next);
    }
  }, [plugins, inspecting]);
  ...
  const visible = useMemo(() => {
    ...
    return plugins.filter((item) => {
      ...
      if (slice === "incidents" && item.catalog?.status === "operational")
        return false;
    ...
    <div className="space-y-6 pt-4">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground max-w-3xl">
          Catalog control for every VaakuOS plugin — including ones the tenant
          app still only declares in the frontend. Offer them, publish outages,
          and inspect usage and issues from one place.
        </p>
      </div>
          ...
          [
            {
              ...
              label: "Known plugins",
              value: stats.total,
              hint: "Catalog + app-declared",
            },
            {
              id: "offered" as const,
              label: "Offered",
              value: `${stats.offered} of ${stats.inCatalog}`,
              hint: "Visible to tenants",
            },
            {
...
function IntegrationCard({
  ...
  const requiredCount =
    row?.config_fields.filter((f) => f.required).length ?? 0;
  const inCatalog = !!row;
...
function ManageDialog({
  ...
  const [updateCatalog, { isLoading: isSaving }] =
    useUpdateIntegrationCatalogMutation();
  ...
  const needsMessage =
    current.status !== "operational" && !current.status_message?.trim();
    ...
    >
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            <code>{integration.provider}</code> — changes here apply to every
            tenant.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-5">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    patch({
                      category: value as IntegrationCatalogEntry["category"],
                    })
                  }
                >
          ...
          <section className="space-y-3 rounded-lg border p-4">
            ...
            <p className="text-xs text-muted-foreground">
              Anything other than Operational shows this note to every tenant
              using {integration.name}. Clearing back to Operational drops the
              note.
            </p>
          </section>
          ...
          <section className="space-y-3">
            ...
            {current.config_fields.length === 0 ? (
              <p className="rounded-md border border-dashed py-6 text-center text-sm text-muted-foreground">
                No fields — this provider connects without tenant-supplied
                credentials.
              </p>
            ) : (
                  ...
                  <div key={index} className="space-y-2 rounded-md border p-3">
                    ...
                    <div className="flex items-center gap-3">
                      <Input
                        value={field.help ?? ""}
                        onChange={(e) =>
                          patchField(index, { help: e.target.value })
                        }
